...
Socket.prototype.addEvent = function (eventName, callback, self, once) {
  if (this.wsCallbackEvent[eventName]) {
    const callbackStatus = this.wsCallbackEvent[eventName].every((eventData) => {
...
Socket.prototype.eventExecute = function (parseData) {
  ...
  } else if (parseData && parseData.method && this.debug) {
...
Socket.prototype.send = function (eventName, param) {
  ...
  if (this.ws && this.ws.readyState == 1) {
    const data = { method: eventName || '' };
    if (param && typeof param === 'object') {
      data.data = param;
    }

    if (self.blobSend) {
      const blod = new Blob([JSON.stringify(data)]);
      self.ws.send(blod);
    } else {
